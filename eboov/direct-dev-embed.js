...
var eboovembed = url.match(/https:\/\/embeddev\.eboov\.com\/(.*)/)[1];
if (eboovembed.indexOf('v=') > -1) {
	const newurl = new URL(document.location.href);
	const params = new URLSearchParams(newurl.search);
	eboovembed = params.get("v");
}else if (url.indexOf('eboov_id=') > -1) {
	const newurl1 = new URL(document.location.href);
	const params1 = new URLSearchParams(newurl1.search);
	eboovembed = params1.get("eboov_id");
}
async function getAllowedDomains(user_id) {
	return new Promise((resolve, reject) => {
		const url = apiUrl+"/v1/domains/get";
...
function getToken(licenseServiceMessage , communicationKeyAsBase64) {
    ...
    //Header
    var header_b64_f = axinomHeaderKey; //constant
...
async function loadVideo(json,show_annotation_time,hide_annotation_time,show_form_time){
    ...
    const communicationKeyId = commKeyId;
    const communicationKeyAsBase64 = commKeyBase;

    const keyId = json.drm_key_id;
    ...
    const axinomMsgToken = getToken(licenseServiceMessage, communicationKeyAsBase64);
    ...
    var playerConfig = {
        // key: "934f0dda-7326-45a3-9039-59bdd62a2099", // license key
        ...
        events: {
            ...
            seek: function(event){
                // const elapsedTimeshift = Math.floor(event.time);
                var seekTime = Math.floor(event.seekTarget);
                if (json.show_form && seekTime >= show_form_time && !isResumeAfterLead) {
                    // this.play();
                    // $('body').append('eventtest2::'+seekTime+'::::'+show_form_time+':::::::'+isResumeAfterLead);
                    this.seek(show_form_time);
                //     this.setViewMode();
                //     exitFullScreen();
                    window.parent.postMessage(
                        { command: "triggerOpenPopup",cta_popup:"false" },
                ...
                // $('body').append('eventtest2::'+elapsedTimeshift+'::::'+show_form_time+':::::::'+isResumeAfterLead);
            },
            ...
            network: {
                ...
                requestConfig.url + "?AxDrmMessage=" + axinomMsgToken;
            },
        },
    };
    var mainSource = {
    	dash: AWSMediaUrl + `/enc/u/${json.uid}/${json.post_id}/manifest.mpd`,
    	hls: AWSMediaUrl + `/enc/u/${json.uid}/${json.post_id}/manifest.m3u8`,
    	poster: posterThumb,
    	analytics: {
    		key: bitmovinAnalyticKey,
    	...
    	drm: {
    		playready: {
    			...
    			headers: {
    				'X-AxDRM-Message': axinomMsgToken
    			}
    		},
    		widevine: {
    ...
    if(json.use_video_in_loop){
      ...
      if (!domainCheck(allowedDomains, parentDomain)) {
      	player.destroy();
      	$('#eboov-display').after('<img src="'+AWSCdnUrl+'/player/web/8/domain.png" style="width:100%;">');
        ...
        $.ajax(Subscsettings).done(function (response) {
        	if(response.status == 'error' && response.message == 'No active subscription'){
        		player.destroy();
        		$('#eboov-display').after('<img src="'+AWSCdnUrl+'/player/web/8/subscription-image.png" style="width:100%;">');
        		$('#eboov-display').remove();
        		return;
        	}else{
        	}
...
function showSpeakerIcon(){
	jQuery('#bmpui-id-2').append('<div class="volume-speaker-icon"><img src="'+AWSCdnUrl+'/player/web/8/video-thumbnail-volume.png"></div>');
}
	...
	if (watermark) {
		if (!showEboov) {
			...
			document.querySelectorAll('.bmpui-ui-watermark').forEach(function(el) {
				...
				el.style.background =
				"url('"+AWSCdnUrl+"/eboov_logo_e.svg')";
